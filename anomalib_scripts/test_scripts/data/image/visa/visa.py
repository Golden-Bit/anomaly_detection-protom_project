...
from anomalib.data.image.visa import Visa, VisaDataset
...
pytorch_dataset_root = Path.cwd() / "datasets" / "Visa" / "visa_pytorch"
output_dir = f"test_scripts/data/image/visa/output_dir"
...
In the current example, we will show how an Anomalib data module can be created for the visa Dataset, and how we can 
...
visa_datamodule.prepare_data()
visa_datamodule.setup()
...
# Train images
i, data = next(enumerate(visa_datamodule.train_dataloader()))
print(data.keys(), data["image"].shape)

# Test images
i, data = next(enumerate(visa_datamodule.test_dataloader()))
...
# Visa Classification Train Set
visa_dataset_classification_train = VisaDataset(
    root=pytorch_dataset_root,
    category="capsules",
    transform=transform,
    split="train",
    task=TaskType.CLASSIFICATION,
)
visa_dataset_classification_train.samples.head()

sample = visa_dataset_classification_train[0]
...
# Visa Classification Test Set
visa_dataset_classification_test = VisaDataset(
    root=pytorch_dataset_root,
    category="capsules",
    transform=transform,
    split="test",
...
sample = visa_dataset_classification_test[0]
print(sample.keys(), sample["image"].shape, sample["image_path"], sample["label"])

"""
Segmentation Task

It is also possible to configure the Visa dataset for the segmentation task, where the dataset object returns image 
and ground-truth mask.
"""

print("\n\n#---SEGMENTATION TASK---#\n")

# Visa Segmentation Train Set
visa_dataset_segmentation_train = VisaDataset(
    root=pytorch_dataset_root,
    category="capsules",
    transform=transform,
    split="train",
...
print("Train samples:")
print(f"{visa_dataset_segmentation_train.samples.head()}\n")

# Visa Segmentation Test Set
visa_dataset_segmentation_test = VisaDataset(
    root=pytorch_dataset_root,
    category="capsules",
    transform=transform,
    split="test",
...
print("Test samples:")
print(f"{visa_dataset_segmentation_test.samples.head()}\n")

sample_index = 20
sample = visa_dataset_segmentation_test[sample_index]

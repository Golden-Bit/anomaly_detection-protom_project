...
from anomalib.data.image.kolektor import Kolektor, KolektorDataset
...
#   to this directory.
dataset_root = Path.cwd() / "datasets" / "Kolektor"
output_dir = f"test_scripts/data/image/kolektor/output_dir"
...
In the current example, we will show how an Anomalib data module can be created for the kolektor Dataset, and how we can 
...
kolektor_datamodule.prepare_data()
kolektor_datamodule.setup()
...
# Train images
i, data = next(enumerate(kolektor_datamodule.train_dataloader()))
print(data.keys(), data["image"].shape)

# Test images
i, data = next(enumerate(kolektor_datamodule.test_dataloader()))
...
# Kolektor Classification Train Set
kolektor_dataset_classification_train = KolektorDataset(
    root=dataset_root,
    #category="bottle",
    transform=transform,
    split="train",
...
kolektor_dataset_classification_train.samples.head()

sample = kolektor_dataset_classification_train[0]
...
# Kolektor Classification Test Set
kolektor_dataset_classification_test = KolektorDataset(
    root=dataset_root,
    #category="bottle",
    transform=transform,
    split="test",
...
sample = kolektor_dataset_classification_test[0]
print(sample.keys(), sample["image"].shape, sample["image_path"], sample["label"])

"""
Segmentation Task

It is also possible to configure the Kolektor dataset for the segmentation task, where the dataset object returns image 
and ground-truth mask.
"""

print("\n\n#---SEGMENTATION TASK---#\n")

# Kolektor Segmentation Train Set
kolektor_dataset_segmentation_train = KolektorDataset(
    root=dataset_root,
    #category="bottle",
    transform=transform,
    split="train",
...
print("Train samples:")
print(f"{kolektor_dataset_segmentation_train.samples.head()}\n")

# Kolektor Segmentation Test Set
kolektor_dataset_segmentation_test = KolektorDataset(
    root=dataset_root,
    #category="bottle",
    transform=transform,
    split="test",
...
print("Test samples:")
print(f"{kolektor_dataset_segmentation_test.samples.head()}\n")

sample_index = 5
sample = kolektor_dataset_segmentation_test[sample_index]
